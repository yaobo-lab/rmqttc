...
async fn mqtt_msg_param_one(Payload(playload): Payload<String>) -> MqttResult {
    log::info!("2. mqtt_msg_param playload------------->:{}", playload);
    Ok(())
}
...
async fn main() {
    ...
    let properties = ConnectProperties {
        user_properties: vec![("Version".to_string(), "1.0.0".into())],
        ..ConnectProperties::default()
    };
    opts.set_connect_properties(properties);
    opts.set_keep_alive(Duration::from_secs(30));
    opts.set_clean_start(false);
    opts.set_credentials("test", "GS+g==");
    ...
    router
        .subscribe(
            "/sub/request/tst/00ab1bd071e0c3f4b0ec92c261cf102",
            mqtt_msg,
            QoS::AtLeastOnce,
        )
        .await
    ...
    router
        .subscribe(
            "/sub/upgrade/00ab1bd07190c3f4b0ec92c261cf102",
            mqtt_msg2,
            QoS::AtLeastOnce,
        )
        .await
